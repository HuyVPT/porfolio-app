...
function FitNavBar() {
    ...
    useEffect(() => {
        ...
        const itemWithUrl = navBarProvider.find((item) => currentUrl.includes(item.path));
        setActiveItem(itemWithUrl || navBarProvider[0]);
    }, []);

    useEffect(() => {
        if (!activeItem) return;
        document.documentElement.style.setProperty('--nav-active-clr', activeItem.activeColor);
        document.documentElement.style.setProperty('--indicator-pos', `${activeItem.id * 70}px`);
    }, [activeItem]);
                        ...
                        {navBarProvider &&
                            navBarProvider.map((item) => (
                                <li
                                    key={item.id}
                                    className={
                                        'nav-bar-item ' + (activeItem && item.id === activeItem.id ? 'active' : '')
                                    }
